...
class App(tk.Frame):
    ...
    class Env(CrawlerEnv):
        def __init__(self, app):
            ...
            #self.ucr_logo = tk.PhotoImage(file=os.path.join(os.path.dirname(os.path.realpath(__file__)), "ucr.png"))
            #sx = int(self.ucr_logo.width()/(self.base_height*0.8))
            #sy = int(self.ucr_logo.height()/(self.base_height*0.8))
            #self.ucr_logo = self.ucr_logo.subsample(sx, sy)
        ...
        def render(self, canvas=None):
            ...
            self.app.world_canvas.create_image(
                ...
                anchor=tk.CENTER, #image=self.ucr_logo,
                tag="UCR"
            )
    ...
    def __init__(self, alg_fn_map, master=None):
        ...
        self.master.title("Crawler -- CS170")
